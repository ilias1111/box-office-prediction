...
def create_batches(cloud_event):
    try:
        ...
        for i in range(0, len(movie_ids), BATCH_SIZE):
            ...
            doc_ref.set({
                f'{timestamp}_{batch_id}': 'published'
            }, merge=True)

        # Set total batches in Firestore
        doc_ref.set({
            f'{timestamp}_total_batches': len(movie_ids) // BATCH_SIZE + 1
        }, merge=True)

        print(f"Created and published {len(movie_ids) // BATCH_SIZE + 1} batches with timestamp {timestamp}")
