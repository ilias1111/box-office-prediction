...
def process_batch(cloud_event):
    ...
    state_ref.set({f"{timestamp}_{batch_id}": "processed"}, merge=True)

    print(
        f"Processed batch {batch_id} of {len(movie_ids)} movie IDs and stored in collection {collection_name}"
    )
