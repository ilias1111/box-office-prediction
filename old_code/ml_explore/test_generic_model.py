    ...
    def train_model(self, data, params, num_boost_round=100, early_stopping_rounds=10):
        ...
        if self.model_type == "lightgbm":
            ...
            self.model = (
                ...
                if params.get("objective") == "Logloss"
    ...
    def predict_and_evaluate(self, X_test, y_test):
        if self.model_type in ["lightgbm", "xgboost"]:
            ...
            raise NotImplementedError(
                f"Model type {self.model_type} is not supported for prediction."
            )

        if (
            self.model_type in ["logistic_regression", "random_forest", "catboost"]
            and params.get("objective") == "Logloss"
        ):
    ...
    def plot_importances(self):
        if self.model_type in ["lightgbm", "xgboost", "catboost"]:
            ...
            plot_feature_importance(
                feature_importances, feature_names, "Feature importance"
            )
        elif self.model_type == "random_forest":
            ...
            print(
                f"Model type {self.model_type} does not support feature importance plotting."
            )
