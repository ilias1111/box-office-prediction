...
import logging
from ml_support_functions import load_data, clean_data, split_dataset, plot_training_history
...
def build_neural_network_model(shape,  optimizer='Adamax', layers = 3, act="elu", neurons=512, dropout=0.8):
    """
...
def train_model(X_train, y_train, X_test, y_test, epochs, batch_size, optimizer, layers, activation, neurons, dropout):
    model_DNN = build_neural_network_model(X_train.shape[1], optimizer, layers, activation, neurons, dropout)
    # Confirm creation of model, and some details
    logging.info(f'Created model with {model_DNN.count_params()} parameters.')
    logging.info(f'Optimizer: {optimizer}, Activation: {activation}, Layers: {layers}, Neurons: {neurons}, Dropout: {dropout}')
...
if __name__ == '__main__':
    ...
    data = clean_data(data_init,
                      columns_to_drop=["revenue_usd_adj_log", "budget_usd_adj_log", 'movie_id',
    ...
    logging.info(f'Cleaned the dataset and removed {len(data) - len(data_init)} records.')
    ...
    logging.info(f'Split the dataset into train and test sets with {len(X_train)} and {len(X_test)} records respectively.')

    model, history = train_model(X_train, y_train, X_test, y_test, epochs=400, batch_size=1024, optimizer='Adamax', layers=4, activation='elu', neurons = 1024, dropout =0.8)
