...
def process_and_store_data(data, cursor, conn):
    ...
    cursor.execute(query, (
        data['id'], data['imdb_id'], data['title'], data['original_title'], data['overview'],
        data['tagline'], data['original_language'], data['status'], data['runtime'],
        data['revenue'], data['budget'], data['poster_path'], release_date,
 ...
 # Insert production companies
    for company in data['production_companies']:
        ...
        ON CONFLICT DO NOTHING;
        """
        cursor.execute(company_query, (data['id'], company['id'], company['name'],
                                       company['logo_path'], company['origin_country']))
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [make_api_request_for_movie(session, movie_id) for movie_id in tmdb_ids_to_retrieve]
    ...
    print(f'Finished processing succesfully {MOVIES_PROCESSED / len(tmdb_ids_to_retrieve) * 100:.2f}% of movies.')
