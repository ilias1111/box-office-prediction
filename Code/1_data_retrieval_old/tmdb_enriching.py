...
async def make_api_request_for_movie_details(session, movie_id):
    ...
    }  # Include credits and release dates in the response
...
async def main():
    MOVIES_PROCESSED = 0
    tmdb_ids_to_retrieve = pd.read_csv("data/processed_data/movie.csv")[
        "movie_id"
    ].tolist()

    with open(
        RELEASE_DATES_CSV, mode="w", newline="", encoding="utf-8"
    ) as release_dates_file, open(
        CAST_CSV, mode="w", newline="", encoding="utf-8"
    ) as cast_file, open(CREW_CSV, mode="w", newline="", encoding="utf-8") as crew_file:
        release_dates_writer = csv.writer(release_dates_file)
        cast_writer = csv.writer(cast_file)
        crew_writer = csv.writer(crew_file)

        release_dates_writer.writerow(
            ["Movie ID", "Country Code", "Release Date", "Certification", "Type"]
        )
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                make_api_request_for_movie_details(session, movie_id, semaphore)
            ...
            for response in responses:
                if response:
                    process_and_store_details(
                        response, release_dates_writer, cast_writer, crew_writer
                    )
                    MOVIES_PROCESSED += 1
    ...
    print(
        f"Finished processing successfully {MOVIES_PROCESSED / len(tmdb_ids_to_retrieve) * 100:.2f}% of movies."
    )
