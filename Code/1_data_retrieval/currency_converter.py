...
def get_monthly_exchange_rates(from_currency: str, to_currency: str) -> Dict[str, float]:
    ...
    if "Time Series FX (Monthly)" in data:
        rates = {k: float(v['4. close']) for k, v in data["Time Series FX (Monthly)"].items()}
        pd.DataFrame(rates.items(), columns=['Date', 'Price']).to_csv(file_path, index=False)
        logging.info(f"Exchange rates for {from_currency} saved to {file_path}")
        return rates
    
    return {}

def get_all_exchange_rates(currency_tuple: Tuple[str, ...]) -> Dict[str, Dict[str, float]]:
    return {currency: get_monthly_exchange_rates(currency, 'USD') for currency in set(currency_tuple) if currency != 'USD'}

def get_rate_for_date(rates: Dict[str, Dict[str, float]], currency: str, date: datetime) -> float:
    if currency == 'USD':
        return 1.0
...
def convert_to_usd(row: pd.Series, rates: Dict[str, Dict[str, float]]) -> float:
    if pd.isna(row['currency']) or pd.isna(row['release_date']) or pd.isna(row['amount']):
...
def process_financial_data(df: pd.DataFrame, output_file: str, columns_pairs: List[Tuple[str, str]]):

    #drop the rows that all the columns are null
    cols = [k for i in columns_pairs for k in i]
    df = df.dropna(subset=cols, how='all')
    df['release_date'] = pd.to_datetime(df['release_date']).dt.to_period('M').dt.to_timestamp()    
    ...
    stats_s = {k: v for k, v in sorted(stats.items(), key=lambda item: item[1]['success_rate'], reverse=True)}
    stats_s.pop('total')
    for currency, stat in stats_s.items():
        logger.info(f"{currency} - Success: {stat['success']}, Fail: {stat['fail']}, Success Rate: {stat['success_rate']:.2f}%")

    # Total number of conversions
    logger.info(f"Total conversions - Success: {stats['total']['success']} Fail: {stats['total']['fail']}, Success Rate: {stats['total']['success_rate']:.2f}%")

if __name__ == "__main__":
    input_file = pd.read_csv('Data/retrieved_data/wikipedia/movie_financial_data.csv').rename({"tmdb_id":"movie_id"}, axis=1)
    movies = pd.read_csv('Data/retrieved_data/tmdb/movies.csv')

    columns_pairs = [('budget_avg', 'budget_currency'), ('box_office_avg', 'box_office_currency')]
    input_file = input_file.merge(movies[['movie_id', 'release_date']], on='movie_id', how='left')
