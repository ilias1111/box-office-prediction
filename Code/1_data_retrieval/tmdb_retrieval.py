...
async def get_movie_details(
    session: aiohttp.ClientSession, tmdb_id: int, semaphore: asyncio.Semaphore
) -> Dict[str, Any]:
    async with semaphore:
        return await make_api_request(
            session,
            f"movie/{tmdb_id}",
            {"append_to_response": "release_dates,external_ids,credits,keywords"},
        )
...
def process_movie_details(data: Dict[str, Any], writers: Dict[str, csv.writer]) -> None:
    ...
    for country_release in data.get("release_dates", {}).get("results", []):
        for release_date in country_release.get("release_dates", []):
            writers["release_dates"].writerow(
                [
                    tmdb_id,
                    country_release.get("iso_3166_1"),
                    release_date.get("release_date"),
                    release_date.get("certification"),
                    release_date.get("type"),
                ]
            )
    ...
    for spoken_language in data.get("spoken_languages", []):
        writers["spoken_languages"].writerow(
            [
                tmdb_id,
                spoken_language.get("iso_639_1"),
                spoken_language.get("english_name"),
            ]
        )
    ...
    for production_company in data.get("production_companies", []):
        writers["production_companies"].writerow(
            [tmdb_id, production_company.get("id"), production_company.get("name")]
        )

    for production_country in data.get("production_countries", []):
        writers["production_countries"].writerow(
            [
                tmdb_id,
                production_country.get("iso_3166_1"),
                production_country.get("name"),
            ]
        )
    ...
    if data.get("belongs_to_collection"):
        writers["collections"].writerow(
            [
                tmdb_id,
                data["belongs_to_collection"].get("id"),
                data["belongs_to_collection"].get("name"),
            ]
        )
    ...
    writers["movies"].writerow(
        [
            ...
            data.get("imdb_id"),
            data.get("external_ids", {}).get("facebook_id"),
            data.get("external_ids", {}).get("instagram_id"),
            data.get("external_ids", {}).get("twitter_id"),
            data.get("external_ids", {}).get("wikidata_id"),
        ]
    )


async def main():
    tmdb_imdb_match = pd.read_csv("data/retrieved_data/tmdb_imdb_match.csv")[
        "tmdb_id"
    ].tolist()
    wikidata_export = pd.read_csv("data/manual_data/wikidata_export.csv")
    tmdb_ids = (
        wikidata_export[wikidata_export["tmdb_id"].notnull()]["tmdb_id"]
        .astype(int)
        .tolist()
    )
    ...
    writers = {
        key: csv.writer(open(file_path, "w", newline="", encoding="utf-8"))
        for key, file_path in CSV_FILES.items()
    }

    writers["release_dates"].writerow(
        ["movie_id", "country_code", "release_date", "certification", "type"]
    )
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [
            get_movie_details(session, tmdb_id, semaphore)
            for tmdb_id in tmdb_ids_to_retrieve
        ]

        processed_count = 0
        for response in tqdm(
            asyncio.as_completed(tasks), total=len(tasks), desc="Processing movies"
        ):
    ...
    logging.info(
        f"Finished processing {processed_count / len(tmdb_ids_to_retrieve) * 100:.2f}% of movies."
    )
