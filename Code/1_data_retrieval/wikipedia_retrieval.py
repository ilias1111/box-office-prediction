        ...
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    retry_after = int(
                        response.headers.get("Retry-After" * 3, RETRY_DELAY)
                    )
                    logging.warning(
                    ...
                    logging.warning(
                        f"Failed to fetch {url}. Status: {response.status}. Attempt {attempt + 1}/{max_retries}"
                    )
...
def parse_financial_info(
    ...
    try:
        ...
        number_scale_pairs = re.findall(
            r"([\d,.]+)(?:\s*[–-]\s*([\d,.]+))?\s*([a-zA-Z]+)?", info_string
        )
...
async def get_movie_info(
    session: aiohttp.ClientSession, url: str, info_types: List[str]
) -> Dict[str, any]:
...
async def process_movie(
    ...
    data: Tuple[int, str],
    info_types: List[str] = ["Budget", "Box office"],
) -> Dict[str, any]:
    tmdb_id, url = data
            ...
            processed_info.update(
                {
                    ...
                    f"{info_type.lower()}_error": info.get("details")
                    if "Error:" in info.get("details", "")
                    else None,
                }
            )
...
async def main():
    try:
        ...
        list_of_movies = wikidata_export[
            (wikidata_export["wikipedia_url"].notnull())
            & (wikidata_export["tmdb_id"].notnull())
        ][["tmdb_id", "wikipedia_url"]].to_records(index=False)  # [:1000]
        ...
        parsed_data_df.to_csv(
            "data/retrieved_data/wikipedia/movie_financial_data.csv", index=False
        )
        logging.info(f"There are {len(parsed_data_df)} rows")
        logging.info(
            f"There are {parsed_data_df['budget_min'].notnull().sum()} cases where the budget is available"
        )
        logging.info(
            f"There are {parsed_data_df['box_office_min'].notnull().sum()} cases where the box office is available"
        )
        logging.info(f"\n{parsed_data_df.head()}")
