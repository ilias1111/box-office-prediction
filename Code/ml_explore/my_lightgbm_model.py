...
class LightGBMRegressor:
    ...
    def train_model(self, data, params, num_boost_round, early_stopping_rounds):
        ...
        X_train, X_test, y_train, y_test = split_dataset(data, target_column=self.target_column)
    ...
    def calculate_feature_importances(self, feature_names, importance_type='split'):
        feature_importance_values = self.model.feature_importance(importance_type=importance_type)
        feature_importances = pd.DataFrame({'feature': feature_names, 'importance': feature_importance_values})
        feature_importances = feature_importances.sort_values(by='importance', ascending=False).reset_index(drop=True)
        return feature_importances

    def plot_importances(self, feature_importances, title, max_num_features=30):
        plot_feature_importance(feature_importances['importance'], feature_importances['feature'], title, max_num_features)
...
if __name__ == '__main__':
    ...
    # Initialize the LightGBM regressor class
        lgb_regressor = LightGBMRegressor(f'code/data_pipeline/data/ml_data_reg_{i}.csv', 
        ...
        # Train the model
        X_test, y_test = lgb_regressor.train_model(data, params, num_boost_round=2000, early_stopping_rounds=50)
