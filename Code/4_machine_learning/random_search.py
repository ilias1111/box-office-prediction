...
def perform_random_search(
    estimator, model_name, X, y, cv, n_iter, scoring, random_state, task_type
):
    param_distributions = MODEL_PARAM_DISTRIBUTIONS.get(model_name, {})

    if (task_type in ("binary_classification", "multiclass_classification")) and (
        model_name not in ["dummy_classifier"]
    ):
