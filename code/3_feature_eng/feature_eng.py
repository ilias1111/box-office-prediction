...
def remove_outliers(movie, remove_outliers_flag):
    try:
        ...
        movie["is_outlier"] = (
            ~movie["is_within_scope"]
            | (movie["ratio_adj"] < roi_bottom_limit)
            | (movie["ratio_adj"] > roi_upper_limit)
        )

        if remove_outliers_flag:
            movie_clean = movie[~movie["is_outlier"]]
            logging.info(
                f"{len(movie) - len(movie_clean)} outliers removed successfully."
            )
            return movie_clean
        else:
...
def check_specific_holiday_delta(date, specific_holiday):
    ...
    future_holidays = {
        k: v for k, v in year_holidays.items() if v == specific_holiday and k >= date
    }
...
def add_holiday_features(movie_df):
    ...
    movie_df["is_on_holiday_window"] = movie_df["release_date"].apply(
        check_holiday_window, days=15, holidays=us_holidays
    )
    movie_df["days_to_xmas"] = movie_df["release_date"].apply(
        check_specific_holiday_delta, specific_holiday="Christmas Day"
    )
    movie_df["days_to_labour_day"] = movie_df["release_date"].apply(
        check_specific_holiday_delta, specific_holiday="Labor Day"
    )
    movie_df["days_to_thanksgiving"] = movie_df["release_date"].apply(
        check_specific_holiday_delta, specific_holiday="Thanksgiving"
    )
...
def pivot_one_hot(df, column_name, prefix, occurrences=30):
    ...
    df_filtered["act_column"] = df_filtered[column_name].apply(
        lambda x: f"{prefix}__{str(x)}"
    )
...
def add_complex_kpi_features(
    ...
    past_production_companies_perfrormance = duckdb.sql(
        past_production_companies_perfrormance_q
    ).to_df()
    ...
    movies_released_within_15_days = duckdb.sql(
        movies_released_within_15_days_q
    ).to_df()

    movie_df = movie_df.merge(
        past_production_companies_perfrormance, on="movie_id", how="left"
    )
    ...
    production_one_hot = pivot_one_hot(
        production_df, "company_name", "is_prod_company", 100
    )
    keyword_one_hot = pivot_one_hot(keyword_df, "keyword_name", "is_keyword", 300)
    collection_one_hot = pivot_one_hot(
        collection_df, "collection_name", "is_collection", 5
    )
    spoken_languages_one_hot = pivot_one_hot(
        spoken_languages, "language", "is_spoken_language", 300
    )
    production_countries_one_hot = pivot_one_hot(
        production_countries, "country_code", "is_prod_country", 300
    )
...
def add_features(
    ...
    if feature_flag == "complex":
        movie = add_complex_kpi_features(
            movie,
            production,
            keyword,
            genre,
...
def add_target_variable(movie_df, task_type):
    movie_df["production_size"] = movie_df["budget_usd_adj"].apply(
        classify_production_size
    )
    ...
    if task_type == "binary_classification":
        ...
        movie_df["multi_class_classification"] = movie_df["roi"].apply(
            classify_fixed_buckets
        )
        movie_df.drop(columns=["revenue_usd_adj"], inplace=True)
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame({"ratio_adj": np.random.normal(loc=0, scale=1, size=100)})
